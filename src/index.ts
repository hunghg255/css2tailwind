...
const getFontSizeDefaultVal = (val: string) => {
      ...
      > = {
        scale: (v: string) => {
          ...
          if (vs[0] === vs[1] || vs.length === 1) {
            return `scale-${
              customTheme.scale?.[vs[0]] ||
              (useAllDefaultValues && scaleDefaultVs[vs[0]]) ||
              `[${vs[0]}]`
            }`;
          }
          return vs
            .map((v, idx) => {
              return `scale-${idx === 0 ? 'x' : 'y'}-${
                customTheme.scale?.[v] ||
                (useAllDefaultValues && scaleDefaultVs[v]) ||
                `[${v}]`
              }`;
            })
            .join(' ');
        ...
        rotate: (v: string) => {
          ...
          if (vs.length > 1) {
            if (
              ...
              return `${t[0]}rotate-${
                customTheme.rotate?.[t[1]] ||
                (useAllDefaultValues && rotateDefaultVs[t[1]]) ||
                `[${t[1]}]`
              }`;
            }
            return undefined;
          ...
          return vs
            .map((v, idx) => {
              ...
              return `${t[0]}translate-${idx === 0 ? 'x' : 'y'}-${
                customTheme.translate?.[t[1]] ||
                (useAllDefaultValues && translateDefaultVs[t[1]]) ||
                `[${t[1]}]`
              }`;
            })
            .join(' ');
          ...
          return vs
            .map((v, idx) => {
              ...
              return `${t[0]}skew-${idx === 0 ? 'x' : 'y'}-${
                customTheme.skew?.[t[1]] ||
                (useAllDefaultValues && skewDefaultVs[t[1]]) ||
                `[${t[1]}]`
              }`;
            })
            .join(' ');
...
export { CssToTailwind, specialAttribute, defaultTranslatorConfig };
