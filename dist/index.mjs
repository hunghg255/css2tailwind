...
const propertyMap = /* @__PURE__ */ new Map([
    ...
    (val) => {
      ...
      const res = vals.map((v) => {
        ...
        pipeV = pipeV.length > 0 ? pipeV : v.replace(/^([a-zA-Z0-9_-]+)\((.+?)\)$/, (_r, k, v2) => {
          canUsePipeV = true;
          return backdropFilterValConfig[k]?.(v2) ?? (canUse = false);
        });
  ...
  [
    ...
    (val) => {
      ...
      const vals = val.split(" ").filter((v) => v !== "").map((v) => isUnit(v) || isColor(v) ? { "transparent": "border-transparent", "currentColor": "border-current", "currentcolor": "border-current" }[val] ?? `border-[${v}]` : propertyMap.get("border-style")[v] ?? "").filter((v) => v !== "").join(" ");
      return vals;
    }
  ],
  [
  ...
  [
    ...
    (val) => {
      ...
      const res = vals.map((v) => {
        ...
        pipeV = pipeV.length > 0 ? pipeV : v.replace(/^([a-zA-Z0-9_-]+)\((.+?)\)$/, (_r, k, v2) => {
          canUsePipeV = true;
          return filterValConfig[k]?.(v2) ?? (canUse = false);
        });
        return canUsePipeV ? pipeV : "";
      });
  ...
  [
    ...
    (val) => {
      const getPipeVal = (val2) => {
        ...
        if (useAllDefaultValues) {
          vals = vals.map((v2) => getRemDefaultVal(v2) ?? `[${v2}]`);
        } else {
          vals = vals.map((v2) => `[${v2}]`);
        }
    ...
    (val) => {
      ...
      const transformValConfig = {
        scale: (v) => {
          ...
          return vs.map((v2, idx) => {
            return `scale-${idx === 0 ? "x" : "y"}-${customTheme.scale?.[v2] || useAllDefaultValues && scaleDefaultVs[v2] || `[${v2}]`}`;
          }).join(" ");
        },
        ...
        rotate: (v) => {
          ...
          if (vs.length > 1) {
            if (vs.length === 3 && ["0", "0deg"].findIndex((v2) => v2 === vs[0]) > -1 && ["0", "0deg"].findIndex((v2) => v2 === vs[1]) > -1) {
              ...
              return `${t2[0]}rotate-${customTheme.rotate?.[t2[1]] || useAllDefaultValues && rotateDefaultVs[t2[1]] || `[${t2[1]}]`}`;
            }
            return void 0;
          }
      ...
      const res = vals.map((v) => {
        ...
        const pipeV = v.replace(/^([a-zA-Z0-9_-]+)\((.+?)\)$/, (_r, k, v2) => {
          canUsePipeV = true;
          const tmpRes = transformValConfig[k]?.(v2) ?? (canUse = false);
...
const getResultCode = (it, prefix = "", config) => {
  ...
  const resultVals = cssCodeList.map((v) => {
    ...
    if (val === "initial" || val === "inherit") {
      ...
      pipeVal = typeof pipe === "function" ? config.customTheme[key.trim()]?.[val] || config.useAllDefaultValues && moreDefaultValuesMap[key.trim()]?.[val] || pipe(val) : config.customTheme[key.trim()]?.[val] || config.useAllDefaultValues && moreDefaultValuesMap[key.trim()]?.[val] || (pipe?.[val] ?? "");
    }
...
const CssToTailwind = (code, config = defaultTranslatorConfig) => {
